    ...
    <div className="flex gap-5">
      {[0, 1, 2, 3].map(i => (
        <div
          ...
          style={{ background: error ? '#ef4444' : i < count ? '#00c9a7' : '#2a2a2a' }}
        />
      ))}
    </div>
  )
...
function Key({ label, onPress, children }) {
  return (
    <button
      onClick={onPress}
      className="w-[72px] h-[72px] rounded-full flex items-center justify-center text-2xl font-light text-white active:opacity-60 transition-opacity"
      style={{ background: label ? '#161616' : 'transparent', border: label ? '1px solid #252525' : 'none' }}
    >
      {children ?? label}
    </button>
  )
